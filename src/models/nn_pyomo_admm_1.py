...
class NeuralODEPyomoADMM:
    ...
    def __init__(self, y_observed, t, first_derivative_matrix, layer_sizes, time_invariant=True, extra_input=None, rho = 1.0,
        ...
        if test_data is not None:
            ...
            self.test_ys = np.array(test_data['y'])
            self.test_ts = np.array(test_data['t'])
            self.test_Xs = np.array(test_data.get('X')) if test_data.get('X') is not None else None
            self.test_Ds = np.array(test_data.get('D')) if test_data.get('D') is not None else None
    ...
    def record_admm_info(self, time_elapsed):
        ...
        if self.test_data is not None:
            y_solution_test = self.node_collocation_pred(
                y0=np.array(self.test_ys)[0],
                t=np.array(self.test_ts),
                D=np.array(self.test_Ds),
                extra_input=np.array(self.test_Xs) if self.test_Xs is not None else None,
            )

            test_pred = np.squeeze(np.array(y_solution_test))
        ...
        y_solution_1 = self.node_diffrax_pred(
            y0=jnp.array(self.y_observed1[0]),
            t=jnp.array(self.t1),
            extra_input=(jnp.array(self.extra_input1), jnp.array(self.t1)) if self.extra_input1 is not None else None,
        )

        y_solution_2 = self.node_diffrax_pred(
            y0=jnp.array(self.y_observed2[0]),
            t=jnp.array(self.t2),
            extra_input=(jnp.array(self.extra_input2), jnp.array(self.t2)) if self.extra_input2 is not None else None,
        )
