...
micromamba run -n "$ENV" python -m src.synthetic_data.training_conv_jax \
  --max_iter '[1000,25000]' --pretrain '[0.2,1]' --layer_width "[3,64,2]" \
  --n_seeds 15 --outdir "$OUTDIR" --data_type "$DATA_TYPE" --penalty_lambda_reg 0.001 --time_invariant False

# Single Pyomo experiments
micromamba run -n "$ENV" python -m src.synthetic_data.training_conv_pyomo \
  --time_invariant False --no_print "False" --n_seeds 15 --outdir "$OUTDIR" --data_type "$DATA_TYPE" \
...
micromamba run -n "$ENV" python -m src.synthetic_data.training_conv_pytorch \
  --max_iter '[400,1000]' --pretrain '[0.2,1]' --layer_width "[3,64,2]" \
  --time_invariant False --n_seeds 15 --outdir "$OUTDIR" --data_type "$DATA_TYPE" --penalty_lambda_reg 0.001