...
class NeuralODEPyomo:
    ...
    def build_model(self):
        ...
        # Initialize ContinuousSet with uneven time points
        ...
        # -------------------------------- ORIGINAL CONSTRAINTS -------------------------------- #
            if M == 1:
                ...
                if self.constraint == "l2":
                    # model.ode.add((nn_y - model.dy_dt[t_i])**2 == 0)
                ...
                if self.constraint == "l2":
                    ...
                    # model.ode.add((nn_y1 - model.dy_dt1[t_i])**2 == 0)
                    # model.ode.add((nn_y2 - model.dy_dt2[t_i])**2 == 0)
                elif self.constraint == "l1":
                    model.ode.add((nn_y1 == dy1_dt))
                    model.ode.add((nn_y2 == dy2_dt))
                    # model.ode.add(nn_y1 == model.dy_dt1[t_i])
                    # model.ode.add(nn_y2 == model.dy_dt2[t_i])
                else:
                    raise ValueError("Constraint should be either 'l1' or 'l2'.")
        
        # -------------------------------- CONSTRAINTS THROUGH A FUNCTION -------------------------------- #
        """def _con1(m, t_i):
            if t_i == m.t.first():
                return Constraint.Skip  # Skip the first time point to avoid boundary issues
            ...
            if self.observed_dim == 1:
                ...
                nn_y1, nn_y2 = self.nn_output(nn_input, m)
                return (nn_y1 == m.dy1_dt[t_i]) and (nn_y2 == m.dy2_dt[t_i])
                #return (nn_y1 == dy1_dt) and (nn_y2 == dy2_dt)

        # Add the constraint to the model
        self.model.con1 = Constraint(self.model.t, rule=_con1)"""
